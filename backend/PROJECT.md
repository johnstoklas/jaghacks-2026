...
- **`POST /api/runs/{run_id}/upload-and-match`** / **`.../upload-and-match-vertex`** — Same video upload; loads `Run.topics`, summarizes, returns `match` + per-topic booleans; **on match**, persists the clip under `data/reels/` like **`POST .../saved-reels`**. Optional **`POST .../saved-reels`** (multipart video) persists without going through match (see README).
